...
const ShortenItem = ({originalUrl, shortUrl, clickCount, createdDate}) => {
    ...
    const [anayticsToggle, setAnalyticsToggle] = useState(false);
    ...
    const analyticsHandler = (shortUrl) => {
        ...
        setAnalyticsToggle(!anayticsToggle);
    }

    const fetchMyShortUrl = async() => {
        setLoader(true);
        try {
            const {data} = await api.get(`/api/urls/analytics/${selectedUrl}?startDate=2025-01-01T20:23:29.062045&endDate=2025-12-31T20:48:14.341619`, {
                headers : {
    ...
    useEffect(()=> {
        if(selectedUrl){
            console.log("🔥 useEffect triggered with:", selectedUrl);
            fetchMyShortUrl();
        }
    },[selectedUrl]);
  ...
  return (
            ...
            <div className="flex-1 sm:space-y-1 max-w-full overflow-x-auto overflow-y-hidden ">
                <div className="text-slate-900 pb-1 sm:pb-0   flex items-center gap-2 ">
                    <a href={`${import.meta.env.VITE_REACT_SUBDOMAIN}/${shortUrl}`}
                        target="_blank"
                        className=" text-[17px]  font-montserrat font-[600] text-blue-600 ">
                        {subDomain + "/" + `${shortUrl}`}
                    </a>
    ...
    <React.Fragment>
        <div className={`${
            anayticsToggle ? "flex" : "hidden"} max-h-96 sm:mt-0 mt-5 min-h-96 relative  border-t-2 w-[100%] overflow-hidden`}>
            {loader ? (
